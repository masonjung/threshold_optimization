...
# hyperparameters
acceptable_disparities =  [ 1.00, 0.50, 0.45, 0.40, 0.35, 0.30, 0.25, 0.20] #, 0.19, 0.18, 0.17, 0.16, 0.15, 0.14, 0.13, 0.12, 0.11, 0.10 ] #[1, 0.5, 0.25, 0.24, 0.239, 0.238, 0.237, 0.236, 0.235, 0.234, 0.233, 0.232, 0.231, 0.23, 0.229, 0.228, 0.227, 0.226, 0.225, 0.223, 0.22, 0.21, 0.2] # [1, 0.5, 0.2, 0.1, 0.01, 0.001]
max_iterations = 10**4
learning_rate = 10**-2
tolerance = 1e-2 #10**-5
...
columns_to_check = ['personality', 'length_feature']#['length_feature','formality_feature', 'sentiment_label', 'personality'] #
...
train_dataset.shape
train_dataset['length_personality'] = train_dataset['length_label'] + '_' + train_dataset['personality']
print(train_dataset.shape)
print(train_dataset.columns)

train_dataset = train_dataset[~train_dataset['length_personality'].isin(['long_neuroticism', 'long_extroversion'])]
...
def generate_groups_and_thresholds(length_groups, formality_groups, sentiment_groups, personality_groups, selected_features):
    ...
    initial_thresholds = {group: initial_thresholds_values for group in unique_groups}
...
def optimize_thresholds(y_true, y_pred_proba, groups, initial_thresholds, group_indices, acceptable_disparity, max_iterations, tolerance, min_acc_threshold, min_f1_threshold,count_group):#,path):
...
features = [uni_personality_groups, uni_length_groups]#, uni_sentiment_groups, uni_personality_groups]
...
# results_path_aux = path+f"//convergence_logout.txt"
#group_labels_path = path+f"//group_labels.txt"
if os.path.exists(results_path):
    os.remove(results_path)
#if os.path.exists(results_path_aux):
#    os.remove(results_path_aux)

count_group = 0
for selected_features in feature_combinations:
    count_group += 1
    with open(results_path, "a") as f:
        f.write(f'{"=" * 70}\nGroup {str(count_group).zfill(2)}: {selected_features}. frac {frac}. Penalty {penalty}\n\n\n')
