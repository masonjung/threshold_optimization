...
def llama_ai_probability(text):
    prompt = (
        ...
        "Estimate the probability that the following text was generated by an AI model. "
        "Respond with only a single probability value between 0 and 1, formatted to 7 decimal places. "
        "Do not provide any additional explanation or text, just the probability.\n\n"
        f"Text:\n{text}\n"
    )
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs,
                ...
                do_sample=False   # Disable sampling to try and get a deterministic response
            )

        response = tokenizer.decode(outputs[0], skip_special_tokens=True).strip()

        # A more flexible regex:
        # This will match a number of the form 0.xxx... up to 7 digits or exactly 1.0000000
        match = re.search(r"\b(0\.\d{1,7}|1\.0000000)\b", response)
        if match:
